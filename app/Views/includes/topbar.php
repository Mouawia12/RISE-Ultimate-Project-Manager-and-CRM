                        ...
                        <li id="topbar-language-dropdown" class="nav-item dropdown hidden-xs">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end language-dropdown">
                                <li>
                                    ...
                                    foreach (get_language_list() as $language_key => $language_label) {
                                        $language_status = "";
                                        $language_text = $language_label;

                                        if ($user_language == strtolower($language_key) || (!$user_language && $system_language == strtolower($language_key))) {
                                            $language_status = "<span class='float-end checkbox-checked m0'></span>";
                                            $language_text = "<strong>" . $language_label . "</strong>";
                                        }

                                        if ($login_user->user_type == "staff") {
                                            echo ajax_anchor(get_uri("team_members/save_personal_language/$language_key"), $language_text . $language_status, array("class" => "dropdown-item clearfix", "data-reload-on-success" => "1"));
                                        } else {
                                            echo ajax_anchor(get_uri("clients/save_personal_language/$language_key"), $language_text . $language_status, array("class" => "dropdown-item clearfix", "data-reload-on-success" => "1"));
                                        }
                                    }
                                    ?>
                                </li>
