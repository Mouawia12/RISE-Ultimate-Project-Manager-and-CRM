...
if (! function_exists('format_number')) {
    ...
    function format_number(float $num, int $precision = 1, ?string $locale = null, array $options = []): string
    {
        ...
        $locale ??= class_exists(Locale::class) ? Locale::getDefault() : 'en';

        // Type can be any of the NumberFormatter options, but provide a default.
        $type = $options['type'] ?? 'decimal';

        // Graceful fallback when ext-intl is unavailable.
        if (! class_exists(NumberFormatter::class)) {
            $formatted = number_format($num, max(0, $precision), '.', ',');

            if ($type === 'currency') {
                $currency = (string) ($options['currency'] ?? '');
                $formatted = trim($currency . ' ' . $formatted);
            }

            if (isset($options['before']) && is_string($options['before'])) {
                $formatted = $options['before'] . $formatted;
            }

            if (isset($options['after']) && is_string($options['after'])) {
                $formatted .= $options['after'];
            }

            return $formatted;
        }

        $type = $type === 'currency' ? NumberFormatter::CURRENCY : NumberFormatter::DECIMAL;
        ...
        if (function_exists('intl_is_failure') && intl_is_failure($formatter->getErrorCode())) {
